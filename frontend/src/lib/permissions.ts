...
export const RESOURCE_BY_PATH: Array<{ match: RegExp; resource: Resource }> = [
  ...
  { match: /^\/app\/(tenants|network|revenue|plans|subscription|integrations)/, resource: "platform" },
  ...
  { match: /^\/app\/(care|care-plans|care-notes|outcomes|documents|practitioner-care-plans|practitioner-clinical-notes|practitioner-evaluations)/, resource: "care" },
  { match: /^\/app\/(medication)/, resource: "medication" },
  { match: /^\/app\/(messages|practitioner-messages|family-messages)/, resource: "messages" },
  { match: /^\/app\/(billing|claims|financial-overview|billing-dashboard|invoices|invoice-builder|finance-claims|claim-tracking|payments|reconciliation|client-funding|revenue-reports|outstanding-balances|payouts)/, resource: "finance" },
  ...
  { match: /^\/app\/(family-overview|family-visit-history|family-care-notes|family-upcoming-visits|family-documents|family-invoices|family-payments|family-billing)/, resource: "family_portal" },
